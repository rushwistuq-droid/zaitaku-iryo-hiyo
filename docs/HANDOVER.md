...
- **適用時期:** 令和8年8月診療分～（2026年制度改正）の月額上限
- **70歳未満:** `getHouseholdHighCostLimit()` が世帯上限を直接適用（区分ア270,300+1%・イ179,100+1%・ウ85,800+1%・エ61,500・オ36,900）
- **70歳以上:** まず個人上限（`getHighCostLimit`：一般22,000／低所得Ⅱ11,000／低所得Ⅰ8,000）→ 任意で世帯上限
- **年間上限:** 8月〜翌7月の還付用。定数 `HIGH_COST_LIMITS_2026.annualCap` に格納し、説明欄に参考表示（月次計算には未適用）
- **多数回該当:** 据え置き（本ツールでは過去12か月の入力なし）
